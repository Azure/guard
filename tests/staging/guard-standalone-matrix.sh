...
cmd_run() {
  ...
  local ns mi
  ns=$(find_ccp_namespace); log "CCP namespace: $ns"
  setup_client "$ns"
  mi=$(read_node_identity); log "node kubelet MI clientId: ${mi:-<none>}"
...
cmd_teardown() {
  local ns; ns=$(find_ccp_namespace 2>/dev/null || echo "")
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --cluster-id) REAL_CLUSTER_ID="$2"; shift 2;;
      --oid)        SUBJECT_OID="$2"; shift 2;;
      --upn)        SUBJECT_UPN="$2"; shift 2;;
